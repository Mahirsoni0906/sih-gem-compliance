...
load_dotenv(os.path.abspath(os.path.join(backend_dir, "..", ".env")))
...
def get_sample_documents():
    ...
    return [
        {
            "id": "vendor_dossier",
            "name": "commercial_vendor_verification_dossier.txt",
            "label": "Vendor Verification Dossier (GEM/2026/B/9012481)",
            "icon": "🏛️",
            "expected_verdict": "LEGITIMATE & 100% COMPLIANT",
            "description": "Full Statutory Dossier: GSTIN, PAN, Udyam, EPFO, ESIC, MII 58.4%, UDIN, OEM Auth, 3-Yr Turnover."
        },
        {
            "id": "gst_valid",
...
async def ai_chat_assistant(payload: Dict[str, Any]):
    ...
    # 1. Mandatory Step 1: Strict Document Air-Gap Enforcement (DPDP Act 2023)
    # GeMMy AI is strictly air-gapped from user-uploaded vendor documents, files, and dossiers.
    air_gap_hit = LocalAIService.check_document_air_gap(user_msg)
    if air_gap_hit:
        add_audit_log(
            user=payload.get("user", "ANONYMOUS_USER"),
            role=user_role,
            action="DOCUMENT_AIRGAP_ENFORCED",
            details=f"User queried: '{user_msg[:60]}...'. Document Air-Gap enforced under DPDP Act 2023.",
            severity="INFO"
        )
        return air_gap_hit

    # 2. Tier 1: Live Internet-Grounded AI Engine (Google Gemini / Sovereign Local LLM)
    # When GEMINI_API_KEY is configured, delivers intelligent, accurate, live-search answers.
    ai_result = await LocalAIService.generate_response(user_msg, user_role, user_context, timeout_seconds=12.0)
    if ai_result:
        add_audit_log(
            user=payload.get("user", "ANONYMOUS_USER"),
            role=user_role,
            action="GEMMY_AI_QUERY",
            details=f"User queried: '{user_msg[:60]}...'. Response generated by {ai_result.get('model')}.",
            severity="INFO"
        )
        return ai_result

    # 3. Tier 2: Instant Direct Statutory Domain Engine Fallback (< 5ms Latency)
    ...
    if direct_match:
        add_audit_log(
            ...
            details=f"User queried: '{user_msg[:60]}...'. Answered by Statutory Domain Engine fallback.",
            severity="INFO"
        )
        return direct_match

    # 4. Tier 3: RAG Retrieval Synthesizer Fallback (< 5ms Latency)
